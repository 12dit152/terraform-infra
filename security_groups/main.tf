...
resource "aws_security_group" "https_public" {
  name        = "allow-https-traffic-from-everywhere"
  ...
  tags = { Name = "https-public" }
}

resource "aws_security_group" "ssh_public" {
  name        = "allow-ssh-access-from-everywhere"
...
resource "aws_security_group" "ssh_http_public" {
  name        = "allow-ssh-and-http-traffic-from-everywhere"
  ...
  tags = {
    Name = "ssh-http-public"
  }
}

resource "aws_security_group" "http_public" {
  name        = "allow-http-traffic-from-everywhere"
...
resource "aws_security_group" "redis_public" {
  name        = "allow-redis-from-everywhere"
  description = "Allow Redis (TCP 6379) from everywhere"
  vpc_id      = var.vpc_id
