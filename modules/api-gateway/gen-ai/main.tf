...
resource "aws_api_gateway_rest_api" "spring_gen_ai_api" {
  name        = "spring-lambda-gen-ai-api"
  description = "API Gateway for Spring Lambda Gen AI function"
...
resource "aws_api_gateway_resource" "proxy" {
  rest_api_id = aws_api_gateway_rest_api.spring_gen_ai_api.id
  parent_id   = aws_api_gateway_rest_api.spring_gen_ai_api.root_resource_id
  path_part   = "{proxy+}"
}
...
resource "aws_api_gateway_integration" "lambda_get" {
  ...
  type                   = "AWS_PROXY"
  uri                    = "arn:aws:apigateway:${data.aws_region.current.id}:lambda:path/2015-03-31/functions/${var.lambda_function_arn_gen_ai}:dev/invocations"
}
...
resource "aws_api_gateway_method" "root_get" {
  rest_api_id   = aws_api_gateway_rest_api.spring_gen_ai_api.id
  resource_id   = aws_api_gateway_rest_api.spring_gen_ai_api.root_resource_id
  http_method   = "GET"
  authorization = "NONE"
}
...
resource "aws_lambda_permission" "api_gw" {
  ...
  function_name = var.lambda_function_name_gen_ai
  qualifier     = var.lambda_alias_name_gen_ai
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.spring_gen_ai_api.execution_arn}/*/*"
}

# Custom Domain Name
resource "aws_api_gateway_domain_name" "custom" {
  domain_name              = var.custom_domain_name_gen_ai
...
resource "aws_api_gateway_base_path_mapping" "custom" {
  api_id      = aws_api_gateway_rest_api.spring_gen_ai_api.id
